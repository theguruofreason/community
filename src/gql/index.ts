...
function lookupPeople(args: object, session: Session): Promise<Person[]> {
    ...
    return session.executeRead<Person[]>(async (tx: ManagedTransaction) => {
        const result: QueryResult<{ [p]: { properties: Person } }> =
            await tx.run(cypher, args);
        return result.records.map((record) => record.get(p).properties);
    });
}

function lookupRelatedEntities(
    primaryId: UUID,
    session: Session,
    args: LookupRelatedEntitiesArgs,
): Promise<EntityAndRelationship[]> {
    const {
        relationshipTypes,
        descriptorSearch,
        direction = "BIDIRECTIONAL",
    } = args;
    const [p, r, s, relationshipOut] = [
        "p" as const,
        "r" as const,
        "s" as const,
        "relOut" as const,
    ];
    const relationshipTypesString: string = relationshipTypes
        ? ":" + relationshipTypes.join("|")
        : "";
    const matchString = `MATCH (${p} {id: $primaryId})-[${r}${relationshipTypesString}]-(${s})`;
    const descriptorString = descriptorSearch
        ? `WHERE ${r}.descriptor =~ '${descriptorSearch}' `
        : "";
    const cypher = `${matchString} ${descriptorString} RETURN (endNode(${r}) = ${s}) as ${relationshipOut}, ${r}, ${s}`;
    return session.executeRead<EntityAndRelationship[]>(
        async (tx: ManagedTransaction) => {
            const result: QueryResult<{
                [relationshipOut]: boolean;
                [r]: { properties: Relationship };
                [s]: EntityResult;
            }> = await tx.run(cypher, { primaryId: primaryId });
...
function lookupEntities(
    args: EntityLookupArgs,
    session: Session,
): Promise<Entity[]> {
    ...
    return session.executeRead<Entity[]>(async (tx: ManagedTransaction) => {
        const result: QueryResult<{ [e]: EntityResult }> = await tx.run(
            cypher,
            args,
        );
...
async function getPostsByAuthorId(
    ...
    return session.executeRead<Post[]>(async (tx: ManagedTransaction) => {
        const result: QueryResult<{ [p]: { properties: Post } }> = await tx.run(
            cypher,
            { ...args, authorId },
        );
        return result.records.map((record) => record.get(p).properties);
...
async function generateFreshPostUUID(
    session: Session,
    retry = 5,
    ...
    const uuid = uuidv4();
    const foundPost: boolean = await session.executeRead(
        async (tx: ManagedTransaction) => {
            return !!(
                await tx.run<Post>(`MATCH (p:Post) {id: $postId} RETURN p`)
            ).records.length;
        },
    );
...
function authorTextPost(
    args: AuthorTextPostArgs,
    session: Session,
): Promise<TextPost> {
    ...
    if (!("activationDateTime" in args))
        args.activationDateTime = args.creationDateTime;
    ...
    return session.executeWrite<TextPost>(async (tx: ManagedTransaction) => {
        const result: QueryResult<{ [tp]: { properties: TextPost } }> =
            await tx.run(cypher, { ...args, postId: postId });
        return result.records[0].get(tp).properties;
    });
}

function establishRelationship(
    args: EstablishRelationshipArgs,
    session: Session,
    ...
    const descriptionString: string = args.descriptor
    ...
    return session.executeWrite<RelationshipPath>(
        async (tx: ManagedTransaction) => {
            const result: QueryResult<{
                [s]: EntityResult;
                [r]: { properties: RelationshipPath };
                [o]: EntityResult;
            }> = await tx.run(cypher, args);
            ...
            return {
                subject: {
                    ...subject.properties,
                    __typename: subject.labels.find((label) =>
                        ENTITY_LABELS.includes(label),
                    ) as EntityLabel,
                },
                object: {
                    ...object.properties,
                    __typename: object.labels.find((label) =>
                        ENTITY_LABELS.includes(label),
                    ) as EntityLabel,
                },
...
interface Context {
    n4jDriver: { session: () => Session };
}

const resolvers = {
    Query: {
        people: (root, args: object, context: Context) =>
            lookupPeople(args, context.n4jDriver.session()),
        posts: (root, args: PostsByAuthorIdArgs, context: Context) =>
            getPostsByAuthorId(
                args.authorId,
                args,
                context.n4jDriver.session(),
            ),
        entities: (root, args: EntityLookupArgs, context: Context) =>
            lookupEntities(args, context.n4jDriver.session()),
    },
    Mutation: {
        authorTextPost: (root, args: AuthorTextPostArgs, context: Context) =>
            authorTextPost(args, context.n4jDriver.session()),
        establishRelationship: (
            root,
            args: EstablishRelationshipArgs,
            context: Context,
        ) => establishRelationship(args, context.n4jDriver.session()),
    },
    Person: {
        posts: (
            author: { id: UUID },
            args: PostsByAuthorIdArgs,
            context: Context,
        ) => getPostsByAuthorId(author.id, args, context.n4jDriver.session()),
        people: (
            primary: { id: UUID },
            args: LookupRelatedEntitiesArgs,
            context: Context,
...
const typeDefs: string = readFileSync(__dirname + "/types.graphql").toString(
    "utf-8",
);
